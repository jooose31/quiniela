...
<?php
    ...
    $query = "SELECT *
    FROM quiniela
    WHERE email ='$correo' and idp='$idp'";
    ...
    $line1 = pg_fetch_array($result);
        ...
        if ($line1) {
            $query3 = "UPDATE quiniela
            SET ge1 = $geu, ge2 = $ged
            WHERE email ='$correo' and idp='$idp'";
            $result3 = pg_query($link, $query3) or die('Query failed: ' . pg_last_error());

            echo"<h1>Aviso: <br />
        La prediccion ha sido editada.</h1>";
        }else {
            $query2 = "INSERT INTO quiniela VALUES ($geu,$ged,0,'$idp','$correo')";
            $result2 = pg_query($link, $query2) or die('Query failed: ' . pg_last_error());
    echo"<h1>Aviso: <br />
        Se agrego la prediccion</h1>";
        }
